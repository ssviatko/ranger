...
#define DUMPERROR 1
#define WORKSIZE 524288 ///< Maximum size of compression segment (48k * golden ratio)
...
void retrieve_range(carith_comp_ctx *ctx, uint8_t a_token, uint32_t *a_start, uint32_t *a_end)
{
	uint32_t l_rangesize = *a_end - *a_start;
	uint32_t l_start_orig = *a_start;

//	printf("rr: passed %08X %08X ", *a_start, *a_end);
	*a_start = (uint64_t)l_start_orig + ((uint64_t)ctx->freq[a_token].count_base * (uint64_t)l_rangesize) / (uint64_t)ctx->plain_len;
	*a_end = (uint64_t)l_start_orig + ((((uint64_t)ctx->freq[a_token].count_base + (uint64_t)ctx->freq[a_token].count) * (uint64_t)l_rangesize) / (uint64_t)ctx->plain_len) - 1;
//	printf("tok %02X - %ld/%ld/%ld  %08X %08X\n", a_token, ctx->freq[a_token].count_base, ctx->freq[a_token].count, ctx->plain_len, *a_start, *a_end);
}

uint8_t token_for_window(carith_comp_ctx *ctx, uint32_t a_window, uint32_t a_start, uint32_t a_end)
{
	size_t i;
	uint32_t l_rangesize = a_end - a_start;
	uint32_t l_windowpos = a_window - a_start;
	uint64_t l_countpos = ((uint64_t)l_windowpos * (uint64_t)ctx->plain_len) / (uint64_t)l_rangesize;
//	printf("l_rangesize %016lX l_windowpos %016lX l_countpos %ld plain_len %ld   ", l_rangesize, l_windowpos, l_countpos, ctx->plain_len);
	...
	uint32_t l_start = a_start;
	uint32_t l_end = a_end;
//	printf("window %08x ", a_window);
	retrieve_range(ctx, i, &l_start, &l_end);
//	printf("checking range for %02lX: %016lX %016lX\n", i, l_start, l_end);
	if (a_window > l_end) {
		do {
			++i; // bump it to the next token that has a count, usually it's a conflict with the top edge of the previous token's range
		} while (ctx->freq[i].count == 0);
		l_start = a_start;
		l_end = a_end;
		retrieve_range(ctx, i, &l_start, &l_end);
		if ((a_window < l_start) || (a_window > l_end)) {
			// not the next one?
			printf("a_window %08X a_start %08X a_end %08X l_start %08X l_end %08X i %ld i.count %ld\n", a_window, a_start, a_end, l_start, l_end, i, ctx->freq[i].count);
			// range calculation tester
			for (i = 0; i < 256; ++i) {
				if (ctx->freq[i].count > 0) {
					uint32_t l_test_lo = a_start;
					uint32_t l_test_hi = a_end;
					retrieve_range(ctx, i, &l_test_lo, &l_test_hi);
					printf("i.count %ld retrieved range for %02lX: %08X %08X\n", ctx->freq[i].count, i, l_test_lo, l_test_hi);
				}
			}
...
void assign_ranges(carith_comp_ctx *ctx, uint64_t a_start, uint64_t a_end)
{
	...
	printf("assign_ranges: lo %016lX hi %016lX size %ld\n", a_start, a_end, a_end - a_start);
	size_t j, l = 0;
		for (j = 0; j < 32; ++j) {
			for (i = 0; i < 8; ++i) {
				size_t k = (j * 8) + i;
				if (ctx->freq[k].count > 0) {
					printf("%02lX: %ld/%ld %010lX %010lX  ", k, ctx->freq[k].count_base, ctx->freq[k].count, ctx->freq[k].range_start, ctx->freq[k].range_end);
					l++;
					if (l == 5) {
						l = 0;
						printf("\n");
...
void process(carith_comp_ctx *ctx)
{
	...
	uint8_t cur_byte;
	uint8_t range_lo_hibyte, range_hi_hibyte;
...
/*
	...
	for (i = 0; i < 256; ++i) {
		if (ctx->freq[i].count > 0) {
			uint64_t l_test_lo = range_lo;
			uint64_t l_test_hi = range_hi;
			retrieve_range(ctx, i, &l_test_lo, &l_test_hi);
			printf("retrieved range for %02lX: %016lX %016lX\n", i, l_test_lo, l_test_hi);
			if (i == 0x65) {
				for (uint64_t w = l_test_lo - 10; w < l_test_lo + 10; ++w) {
					token_for_window(ctx, w, range_lo, range_hi);
				}
				for (uint64_t w = range_lo; w < range_lo + 5; ++w) {
					token_for_window(ctx, w, range_lo, range_hi);
				}
				for (uint64_t w = range_hi - 5; w < range_hi; ++w) {
					token_for_window(ctx, w, range_lo, range_hi);
				}
			}
		}
...
//		printf("pos %ld read %02X new range_lo %010lX range_hi %010lX\n", plain_ptr, cur_byte, range_lo, range_hi);
		range_lo_hibyte = (range_lo >> 24);
		range_hi_hibyte = (range_hi >> 24);
...
//				printf("underflow detected: %02X/%02X %d times\n", underflow_lo, underflow_hi, underflow_ctr);
				...
				uint32_t range_lo_savetop = range_lo & 0xff000000;
				uint32_t range_hi_savetop = range_hi & 0xff000000;
				range_lo <<= 8;
				range_hi <<= 8;
				range_hi |= 0xff;
				range_lo &= 0x00ffffff;
...
//	printf("hi %016lX lo %016lx mid %016lx\n", range_hi, range_lo, l_mid);
	...
	for (i = 0; i < 4; ++i) {
		range_lo_hibyte = (range_lo >> 24) & 0xff;
...
//			underflow_hi = range_hi_hibyte;
			...
			uint8_t range_hi_2ndbyte = (range_hi >> 16) & 0xff;
			if ((range_lo_2ndbyte == 0xff) && (range_hi_2ndbyte == 0x00)) {
//				printf("underflow detected: %02X/%02X win %08X\n", underflow_lo, underflow_hi, window);
				// save the high byte of the ranges and the window
				uint32_t range_lo_savetop = range_lo & 0xff000000;
				uint32_t range_hi_savetop = range_hi & 0xff000000;
				uint32_t window_savetop   =   window & 0xff000000;
...
//		}
	...
	if (DUMPERROR && (compare != 0)) {
		...
		close(err_fd);
		int errc_fd = open("error-comp", O_RDWR | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
		write(errc_fd, ctx->comp, ctx->comp_len);
		close(errc_fd);
		int errd_fd = open("error-decomp", O_RDWR | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
		write(errd_fd, ctx->decomp, ctx->decomp_len);
		close(errd_fd);
		printf("********* ROYAL FUCKUP *********************\n");
		exit(EXIT_FAILURE);
	}
